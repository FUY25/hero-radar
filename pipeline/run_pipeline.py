...
def collect_trending_repos_movers(settings: dict[str, Any], fetched_at: str) -> list[SourceItem]:
    ...
    sparkline_by_repo: dict[str, tuple[list[Any], str]] = {}
    for period, rows in repositories.items():
        for row in rows:
            full_name = str(row.get("fullName") or "")
            sparkline = row.get("sparkline") if isinstance(row.get("sparkline"), list) else []
            if full_name and sparkline and full_name not in sparkline_by_repo:
                sparkline_by_repo[full_name] = (sparkline, str(period))
    ...
    for period in periods:
        ...
        for row in rows[:limit]:
            ...
            sparkline = row.get("sparkline") if isinstance(row.get("sparkline"), list) else []
            sparkline_source_period = period
            if not sparkline and full_name in sparkline_by_repo:
                sparkline, sparkline_source_period = sparkline_by_repo[full_name]
            items.append(
                SourceItem(
                    ...
                    metadata={
                        ...
                        "sparkline": sparkline,
                        "sparkline_source_period": sparkline_source_period,
...
<body>
  ...
  <script>
    ...
    let sparklineFallbackCache = null;
    function sparklineFallbackMap() {
      if (sparklineFallbackCache) return sparklineFallbackCache;
      const map = new Map();
      for (const item of data.items || []) {
        if (item.channel !== 'github_movers_trending_repos') continue;
        const row = item.metadata?.sparkline;
        if (!Array.isArray(row) || !row.length) continue;
        const key = String(item.name || '').toLowerCase();
        if (!key || map.has(key)) continue;
        map.set(key, {values: row, period: item.metadata?.sparkline_source_period || item.metadata?.period || ''});
      }
      sparklineFallbackCache = map;
      return sparklineFallbackCache;
    }
    function sparklinePayload(item, value) {
      if (Array.isArray(value) && value.length) {
        return {values: value, period: meta(item, 'sparkline_source_period') || meta(item, 'period') || '', reused: false};
      }
      const fallback = sparklineFallbackMap().get(String(item.name || '').toLowerCase());
      return fallback ? {values: fallback.values, period: fallback.period, reused: true} : null;
    }
    ...
    const columnsByChannel = {
      ...
      github_movers_trending_repos: [rank('TR 排名', 'Trending Repos 自己的当前时间范围 momentum 榜原始顺序。切换 24h / 7d / 30d 会进入不同榜单，并从 1 重新开始；它不是我们二次排序的结果。'), name('Repo'), desc(), c('star 增量曲线', 'Trending Repos 原生 sparkline 数组，页面用它画近期新增 star 曲线。我们按当前筛选行里的最大增量统一缩放，右侧数字是最后一个点；若当前窗口没给曲线，会复用同 repo 在同一 source payload 中给出的原生曲线。', 'm.sparkline', 'spark', 'sparkline'), c('总 star', 'Trending Repos 原生 starsCount，表示当前仓库总 star。它说明体量，不等于增长速度。', 'm.stars_count', 'num', 'num'), c('总 fork', 'Trending Repos 原生 forksCount，表示当前仓库总 fork。fork 往往比 star 更偏开发者实际尝试，但也可能来自模板或镜像。', 'm.forks_count', 'num', 'num'), c('TR 动量分', 'Trending Repos 原生 score。官方说明大致由 star delta 的 EMA、fork delta 的 EMA、freshness_bonus 组合而成，用来排它自己的 momentum 榜；越高表示它认为近期动量越强，不是我们的最终评分。', 'm.source_score', 'num', 'num'), c('star 动量', 'Trending Repos 原生 scoreComponents.starsVelocity，表示近期 star 增量的平滑动量分量。它不是简单 stars/hour；越高表示近期 star 增长越快。', 'm.stars_velocity', 'num', 'num'), c('fork 动量', 'Trending Repos 原生 scoreComponents.forksVelocity，表示近期 fork 增量的平滑动量分量。fork 增长更像开发者试用信号，越高越值得看。', 'm.forks_velocity', 'num', 'num'), c('新项目加成', 'Trending Repos 原生 scoreComponents.freshnessBonus，新 repo 的 recency 加成，会随 repo age 衰减。高值说明项目较新或被 source 判定更 fresh；它不是质量分，只是新鲜度修正。', 'm.freshness_bonus', 'num', 'num'), c('主题', 'Trending Repos 返回的 GitHub topics。用于快速判断领域；topic 是仓库维护者/平台标签，不是我们分类。', 'm.topics', '', 'list'), detail()],
      github_movers_repofomo: [rank('范围排名', 'RepoFOMO 当前原生范围内的排名。切换 7d / 30d / 60d 后，分别按 7d_new / 30d_new / 60d_new 从高到低重新编号；这不是我们加权，只是把 RepoFOMO 同一张表里的原生窗口字段作为 lens。'), name('Repo'), desc(), c('一句话', 'RepoFOMO 原生 info/pitch 文案，通常是对仓库用途的短描述。用于快速理解项目，不参与本地计算。', 'm.info', 'desc'), c('总 star', 'RepoFOMO 原生 tot_stars，表示当前总 star。体量越大说明已有关注越多，但不代表它现在仍在加速。', 'm.stars_total', 'num', 'num'), c('7d 新增 star', 'RepoFOMO 原生 7d_new，表示过去 7 天新增 star 数。它是周级 mover 信号；越高说明最近一周增长越强。', 'm.stars_7d', 'num', 'num'), c('30d 新增 star', 'RepoFOMO 原生 30d_new，表示过去 30 天新增 star 数。它比 7d 更平滑，适合看月级持续增长。', 'm.stars_30d', 'num', 'num'), c('60d 新增 star', 'RepoFOMO 原生 60d_new，表示过去 60 天新增 star 数。它更偏中期趋势，适合区分短期尖峰和持续升温。', 'm.stars_60d', 'num', 'num'), c('7d 增长率', 'RepoFOMO 原生 7d% 增长率，表示过去 7 天 star 的相对增长。小 repo 容易很高，所以要和 7d 新增一起看。', 'm.growth_7d_percent', 'num', 'num'), c('30d 增长率', 'RepoFOMO 原生 30d% 增长率，表示过去 30 天 star 的相对增长。用于看月级相对加速，小体量项目可能被放大。', 'm.growth_30d_percent', 'num', 'num'), c('总 fork', 'RepoFOMO 原生 forks，表示当前 fork 总数。它是开发者复制/尝试的粗信号，不等同于活跃用户。', 'm.forks', 'num', 'num'), c('新增 fork', 'RepoFOMO 原生 new_forks，表示近期新增 fork。和新增 star 一起看，可以判断是否只是围观，还是有人开始动手试。', 'm.new_forks', 'num', 'num'), c('repo 年龄(天)', 'RepoFOMO 原生 star_age，单位天，用作新旧程度参考。数值越小越新；新项目增长快更值得关注，但也更容易是假尖峰。', 'm.star_age_days', 'num', 'num'), detail()],
      github_search: [rank('搜索排名', 'GitHub Search API 在当前 query 里的返回顺序。我们请求 sort=stars、order=desc，所以它基本按总 star 排；这不是 trending，也不是新增速度。'), name('Repo'), desc(), c('搜索词', '命中这行的 GitHub Search 配置 query，例如 agent stars:>20。这个字段说明它是被哪个主动搜索入口抓进来的；改 query 后下一次 run 生效。', 'm.query_label'), c('总 star', 'GitHub REST API 的 stargazers_count，当前总 star 数。它是体量指标，不是最近新增。', 'm.stars', 'num', 'num'), c('总 fork', 'GitHub REST API 的 forks_count，当前总 fork 数。fork 更偏开发者尝试/复用信号，但不代表近期增速。', 'm.forks', 'num', 'num'), c('watchers(≈star)', 'GitHub API 原生 watchers_count。GitHub 这个字段常和 star 口径接近，不建议当成独立趋势指标；主要用于完整保留 API 事实。', 'r.watchers_count', 'num', 'num'), c('open issues', 'GitHub API open_issues_count，通常包含 open issues 和 pull requests。高值可能表示活跃，也可能表示维护压力，需要结合 repo 内容看。', 'r.open_issues_count', 'num', 'num'), c('license', 'GitHub API license 字段，通常是 SPDX id 或 license 名称。用于判断能否复用/研究，不是热度信号。', 'r.license'), c('创建时间', 'GitHub created_at，仓库创建时间。新仓库高增长更接近早期机会；老仓库高 star 更多是存量体量。', 'm.created_at', '', 'date'), c('更新时间', 'GitHub updated_at，仓库元数据或内容最近更新时间。只能说明近期有变化，不一定是代码 push。', 'r.updated_at', '', 'date'), c('最近 push', 'GitHub pushed_at，最近一次代码 push 时间。比 updated 更接近开发活跃度；很久没 push 的项目即使 star 高也要谨慎。', 'm.pushed_at', '', 'date'), c('主题', 'GitHub topics，仓库维护者/平台标签。用于快速判断领域和关键词，不是我们自动分类。', 'm.topics', '', 'list'), detail()],
      hn_search: [rank('搜索排名', 'HN Algolia search_by_date 在当前 query 和时间范围里的返回顺序。切换 24h / 7d / 30d 会进入不同搜索窗口，并从 1 重新开始；这个 endpoint 偏按时间返回搜索命中，不是按 points 排名。'), name('HN/URL'), desc(), c('搜索词', '命中这行的 HN Algolia query，例如 agent。它说明这个 HN story 是由哪个主动搜索词抓到的；改 query 后下一次 run 生效。', 'm.query_label'), c('HN 分数', 'HN Algolia 原生 points，也就是 HN story 当前得分。分数越高说明 HN 社区投票越强，但它会受时间、标题和社区偏好影响。', 'm.points', 'num', 'num'), c('评论数', 'HN Algolia 原生 num_comments，表示评论数。评论多说明讨论强，但可能是争议、质疑或负面反馈；需要读评论。', 'm.comments', 'num', 'num'), c('作者', 'HN author，提交这个 story 的 HN 用户名。可用于判断是否是项目作者、熟悉账号或普通转发者。', 'm.author'), c('发布时间', 'HN Algolia created_at，story 创建时间。结合 points/comments 和时间窗判断热度是否刚发生。', 'm.created_at', '', 'date'), c('HN 链接', 'Hacker News item 页面链接。打开后可以看完整评论区；后续 LLM 深挖也应从这里拉 comments。', 'm.hn_url', '', 'url'), detail()],
      hn_top: [rank('榜单排名', 'HN Firebase list 内的原始位置。topstories/newstories/beststories 各自都有自己的顺序；数值越小表示在该 list 里越靠前。'), name('HN/URL'), desc(), c('榜单', 'HN Firebase 原生列表来源：topstories、newstories 或 beststories。top 偏当前热度，new 偏最新，best 偏长期质量。', 'm.list'), c('HN 分数', 'HN Firebase item.score，HN story 当前分数。越高表示投票越强；要结合发布时间看，老帖天然更容易累积分。', 'm.score', 'num', 'num'), c('评论数', 'HN Firebase item.descendants，表示该 story 的评论总数。它不是情绪分；高评论可能是兴奋，也可能是争议。', 'm.comments', 'num', 'num'), c('作者', 'HN Firebase item.by，提交这个 story 的 HN 用户名。可辅助判断是不是项目作者或高信号账号。', 'm.author'), c('发布时间', 'HN Firebase item.time，Unix 秒时间戳；这里转换成 UTC 时间。用于判断上榜速度和是否新发生。', 'm.created_at_unix', '', 'unix'), c('HN 链接', 'Hacker News item 页面链接。评论树入口在 raw.kids，主表不展示；需要深挖时再拉评论。', 'm.hn_url', '', 'url'), detail()],
      product_hunt: [rank('PH 排名', 'Product Hunt tab 的固定顺序。采集请求使用 order=VOTES，并优先使用 dailyRank；数值越小越靠前。另看 daily rank / weekly rank 判断日榜和周榜位置。'), name('产品'), desc(), c('票数', 'Product Hunt GraphQL votesCount，当前投票数。它是 PH 社区的启动当天/近期关注信号，不等同于真实使用。', 'm.votes', 'num', 'num'), c('评论数', 'Product Hunt GraphQL commentsCount，当前评论数。评论多说明讨论更多，但需要看内容判断是赞同、提问还是质疑。', 'm.comments', 'num', 'num'), c('日榜排名', 'Product Hunt dailyRank，产品在 PH 日榜的原生排名。数值越小越靠前；为空时说明 API 没返回该字段。', 'm.daily_rank', 'num', 'num'), c('周榜排名', 'Product Hunt weeklyRank，产品在 PH 周榜的原生排名。比 daily 更平滑，但更滞后。', 'm.weekly_rank', 'num', 'num'), c('创建时间', 'Product Hunt createdAt，PH post 创建时间。用于判断它是不是刚发布。', 'm.created_at', '', 'date'), c('Featured 时间', 'Product Hunt featuredAt，PH featured 时间。featured 后票数可能突然增加，所以看增长时要注意这个事件。', 'm.featured_at', '', 'date'), c('官网', 'Product Hunt 返回的产品官网链接。用于看真实产品，不是 PH 页面。', 'm.website', '', 'url'), detail()],
      huggingface_models: hfCols('Model'),
      huggingface_datasets: hfCols('Dataset'),
      huggingface_spaces: hfCols('Space'),
      npm_search: [rank('搜索排名', 'npm registry search API 在当前 query 里的返回顺序。npm 的排序由文本匹配和 score 共同决定，不是下载量榜，也不是增长榜；看 weekly/monthly downloads 才是使用量。'), name('Package'), desc(), c('搜索词', '命中这行的 npm registry search query，例如 mcp 或 ai agent。它决定这个包为什么被抓进来；改 query 后下一次 run 生效。', 'm.query_label'), c('版本', 'npm package.version，当前包版本号。版本高低本身不代表质量，要结合更新时间和下载看。', 'm.version'), c('周下载', 'npm search API downloads.weekly，过去一周下载量。它是使用/依赖热度的粗信号；包管理器自动安装会放大成熟包。', 'm.weekly_downloads', 'num', 'num'), c('月下载', 'npm search API downloads.monthly，过去一月下载量。比周下载更平滑，但对刚冒头项目反应更慢。', 'm.monthly_downloads', 'num', 'num'), c('被依赖数', 'npm search API dependents，依赖这个包的包数量。高值表示生态嵌入深，但新项目通常还很低。', 'm.dependents', 'num', 'num'), c('npm 搜索分', 'npm search API 的 score.final 或 searchScore，是 npm 自己的搜索排序分。它混合文本匹配、质量、流行度、维护等因素；不是下载速度。', 'm.score_final', 'num', 'num'), c('质量分', 'npm search API score.detail.quality，npm 自己的质量子分。通常和包元数据、测试/文档等健康度相关；不是我们计算。', 'm.score_quality', 'num', 'num'), c('流行度分', 'npm search API score.detail.popularity，npm 自己的流行度子分。通常受下载量、dependents 等影响；成熟包会更占优。', 'm.score_popularity', 'num', 'num'), c('维护分', 'npm search API score.detail.maintenance，npm 自己的维护子分。通常反映最近发布和维护活跃；低值可能表示不活跃。', 'm.score_maintenance', 'num', 'num'), c('license', 'npm package.license。用于判断复用风险，不是热度或质量分。', 'm.license'), c('关键词', 'npm package.keywords，包作者设置的关键词。用于快速判断领域；可能缺失或营销化。', 'm.keywords', '', 'list'), c('更新时间', 'npm package.date，npm 返回的包更新时间。新近更新配合下载增长更值得看。', 'm.package_date', '', 'date'), detail()],
    ...
    function hfCols(label) {
      return [rank('HF 排名', 'Hugging Face trending API 的原始返回顺序。HF 按自己的 trendingScore 排资源；我们只保留它的排序和字段，不把它和 GitHub、HN 做加权。'), name(label), desc(), c('HF 趋势分', 'Hugging Face API 原生 trendingScore。HF 用它做 trending 排序，具体公式不公开；把它当 HF 平台自己的趋势信号，不要和 GitHub star 直接比较。', 'r.trendingScore', 'num', 'num'), c('点赞', 'Hugging Face likes，当前点赞数。它是平台内关注度，不等于下载或真实使用。', 'm.likes', 'num', 'num'), c('下载量', 'Hugging Face downloads，当前下载量。对模型/数据集更有意义；Spaces 有时为空或口径不同。', 'm.downloads', 'num', 'num'), c('任务/类型', 'Hugging Face pipeline_tag，表示模型/资源类型或任务，例如 text-generation。用于判断领域，不是热度指标。', 'm.pipeline_tag'), c('库/SDK', 'Hugging Face 原生 library_name 或 Spaces sdk。它说明技术栈，例如 transformers、gradio、docker。', 'r.library_name'), c('创建时间', 'Hugging Face createdAt，资源创建时间。新资源上 trending 更接近早期信号。', 'm.created_at', '', 'date'), c('修改时间', 'Hugging Face lastModified，最近修改时间。近期修改配合 trendingScore 更值得看。', 'm.last_modified', '', 'date'), c('标签', 'Hugging Face tags，平台标签。用于快速判断任务、框架和领域。', 'm.tags', '', 'list'), detail()];
    }
    function pypiCols(label) {
      return [rank('RSS 排名', 'PyPI RSS feed 的原始条目顺序。newest feed 越靠前表示越新发布，updates feed 越靠前表示越新更新；它不是下载量、质量或增长排名。'), name('Package'), desc(), c('feed 类型', 'PyPI RSS feed 来源：newest packages 或 latest updates。newest 看新包，updates 看最近发布。', 'm.feed'), c('版本', 'RSS title 里解析出的 release version。newest feed 可能没有版本，updates feed 通常有。', 'm.version'), c('最新版本', '从 PyPI JSON API info.version enrich 得到的当前最新版本。为了速度只 enrich 前 N 条；空值通常表示超出 enrich 上限或 API 未返回。', 'm.latest_version'), c('发布时间', 'PyPI RSS pubDate，表示该 package 或 release 出现在 feed 的时间。用来判断是否刚发布/刚更新。', 'm.pub_date', '', 'date'), c('Python 版本', 'PyPI JSON info.requires_python，包要求的 Python 版本范围。用于判断可用性，不是热度指标。', 'm.requires_python'), c('license', 'PyPI JSON info.license，包声明的 license。用于复用风险判断，可能为空或非标准。', 'm.license'), c('关键词', 'PyPI JSON info.keywords，包作者填写的关键词。用于快速扫领域，可能不规范。', 'm.keywords'), c('分类', 'PyPI JSON classifiers 前几项。它是包作者选择的标准分类，适合判断框架、Python 版本、主题。', 'm.classifiers', 'desc', 'list'), c('项目链接', 'PyPI JSON project_urls，通常包含 Homepage、Repository、Docs、Issues。后续 LLM/agent 深挖应优先打开这些链接。', 'm.project_urls', 'desc', 'object'), detail()];
    }
    ...
    const sortOptionsByChannel = {
      ...
      github_movers_trending_repos: [['native', '原生顺序', '$rank', 'asc'], ['source_score', 'TR 动量分', 'm.source_score', 'desc'], ['stars_velocity', 'star 动量', 'm.stars_velocity', 'desc'], ['forks_velocity', 'fork 动量', 'm.forks_velocity', 'desc'], ['freshness', '新项目加成', 'm.freshness_bonus', 'desc'], ['stars_count', '总 star', 'm.stars_count', 'desc']],
      ...
      github_search: [['native', '搜索顺序', '$rank', 'asc'], ['stars', '总 star', 'm.stars', 'desc'], ['forks', '总 fork', 'm.forks', 'desc'], ['open_issues', 'open issues', 'r.open_issues_count', 'desc'], ['updated', '更新时间', 'r.updated_at', 'desc']],
      hn_search: [['native', '搜索顺序', '$rank', 'asc'], ['points', 'HN 分数', 'm.points', 'desc'], ['comments', '评论数', 'm.comments', 'desc'], ['created', '发布时间', 'm.created_at', 'desc']],
      hn_top: [['native', '榜单顺序', '$rank', 'asc'], ['score', 'HN 分数', 'm.score', 'desc'], ['comments', '评论数', 'm.comments', 'desc'], ['created', '发布时间', 'm.created_at_unix', 'desc']],
      product_hunt: [['native', 'PH 顺序', '$rank', 'asc'], ['votes', '票数', 'm.votes', 'desc'], ['comments', '评论数', 'm.comments', 'desc'], ['daily_rank', '日榜排名', 'm.daily_rank', 'asc'], ['weekly_rank', '周榜排名', 'm.weekly_rank', 'asc']],
      huggingface_models: [['native', 'HF 顺序', '$rank', 'asc'], ['trendingScore', 'HF 趋势分', 'r.trendingScore', 'desc'], ['downloads', '下载量', 'm.downloads', 'desc'], ['likes', '点赞', 'm.likes', 'desc']],
      huggingface_datasets: [['native', 'HF 顺序', '$rank', 'asc'], ['trendingScore', 'HF 趋势分', 'r.trendingScore', 'desc'], ['downloads', '下载量', 'm.downloads', 'desc'], ['likes', '点赞', 'm.likes', 'desc']],
      huggingface_spaces: [['native', 'HF 顺序', '$rank', 'asc'], ['trendingScore', 'HF 趋势分', 'r.trendingScore', 'desc'], ['likes', '点赞', 'm.likes', 'desc']],
      npm_search: [['native', '搜索顺序', '$rank', 'asc'], ['weekly', '周下载', 'm.weekly_downloads', 'desc'], ['monthly', '月下载', 'm.monthly_downloads', 'desc'], ['score', 'npm 搜索分', 'm.score_final', 'desc'], ['dependents', '被依赖数', 'm.dependents', 'desc']],
    ...
    function renderCell(item, column, index) {
      ...
      if (column.kind === 'sparkline') return sparklineHtml(item, value);
    ...
    function currentSparklineMax() {
      ...
      windowedRows().forEach(item => {
        const payload = sparklinePayload(item, meta(item, 'sparkline'));
        if (payload && Array.isArray(payload.values)) payload.values.forEach(value => {
          const num = Number(value);
    ...
    function sparklineHtml(item, value) {
      const payload = sparklinePayload(item, value);
      if (!payload || !Array.isArray(payload.values) || !payload.values.length) return '';
      const nums = payload.values.map(Number).filter(Number.isFinite);
      ...
      const periodNote = payload.period ? `；曲线来源=${payload.period}${payload.reused ? '（同 repo 复用）' : ''}` : '';
      const title = `star 增量曲线: ${nums.map(num => fmt(num)).join(' / ')}；统一缩放 max=${fmt(max)}${periodNote}`;
