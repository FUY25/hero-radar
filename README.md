...
- `schema_smoke` 覆盖 OpenClaw、Hermes Agent、HeyClicky、generic chatbot、funding/news、standalone model、tutorial/resource list、ordinary dashboard/editor/calculator、gray-zone utility，用来快速验证固定 case、输出 schema 和聚合逻辑是否自洽。
- OpenClaw / Hermes / HeyClicky 的候选相关性可以是 high，但只凭 supplied first-party evidence 时，实际 publication score 允许落在 medium；`candidate_relevance` 与 `publication_readiness` 分开评估。
- generic chatbot、纯新闻、单独模型发布、教程资源、普通 dashboard/editor/calculator 必须低分，除非证据明确展示了新的 workflow unlock。
- `provider_smoke` 用少量真实 Kimi 调用验证 provider 连通性、JSON schema 和基本响应形状；它不替代生产等价 Agent eval。
- 20-30 candidate bounded run 用来验证真实 feed routing、cache、tool budgets、API payload 和 UI 展示。

当前固定 scorer corpus 包含 20 个 case。旧静态路径只称为 `schema_smoke`，不能把 fixture-authored response 的通过率当成生产 Agent 质量结论。V2 case contract 对每个 case 显式声明 `tool_policy=forbidden|optional|required`：禁止调查的 rich-context case 不暴露工具；required case 只暴露 gold-authorized 的生产 ToolSpec，并必须有可命中的 replay recording。

生产等价的 Layer 2 component eval 只评估当前生产 V2。它使用 `scoring_cases.v2.jsonl` 和 deterministic、network-free replay tools，固定运行 20 个 case，并直接调用生产 `score_with_investigator()` 与独立 Chinese Brief Writer seam。Case contract 会拒绝 required tool 没有 recording、forbidden policy 仍允许工具、以及 `should_print` 与 `publication_readiness` 冲突的配置。先验证 20 个 case 的严格契约：

```bash
python3 -m pipeline.decision.run_layer2_component_eval --validate-only
```
...
汇总同时报告四个维度，避免把“最终决策错”和“多调用一次工具”压成同一个失败率：`execution`（执行、preflight、telemetry）、`decision`（score、route）、`efficiency`（tool trajectory、stopping/repair）、`safety`（evidence refs、grounding、known gaps、Brief、prompt injection）。失败或 unavailable tool observation 不进入 Brief 的 `project_facts`，只作为中性的 `information_gaps` 传递。
...
输出包含 latest-slot results JSONL、append-only attempts JSONL、aggregate JSON、V2 trial-stability Markdown report、checkpoint、run metadata，以及需要 Brief 的 opaque blind-review artifacts。只有固定 20 case × 3 trials、Brief 开启、production 配置和真实 Kimi 同时满足时才标记 `release_full` / `release_eligible=true`；`--case-id` sentinel 自动标记为 `partial_debug`。`--provider-factory` 仅供 deterministic tests 使用，其 artifact 会标记 `test_provider`，不能作为 release evidence。退出码 `2` 表示 dataset/config 错误，`3` 表示运行级 infrastructure 或 artifact 错误，`4` 表示完整运行结束但一个或多个 case/trial grader 未通过。单个 case/trial 的 provider、tool、validation 或 repair 失败属于可恢复的评估结果，不使用退出码 `3` 提前终止全量运行。
