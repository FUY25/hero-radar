...
当前固定 scorer corpus 包含 20 个 case。旧静态路径只称为 `schema_smoke`，不能把 fixture-authored response 的通过率当成生产 Agent 质量结论。

生产等价的 Layer 2 component eval 只评估当前生产 V2。它使用版本化 JSONL 和 deterministic、network-free replay tools，固定运行 20 个 case，并直接调用生产 `score_with_investigator()` 与独立 Chinese Brief Writer seam。先验证 20 个 case 的严格契约：
