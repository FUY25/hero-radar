...
生产等价的 Layer 2 component eval 使用版本化 JSONL、离线 replay tools，并直接调用生产 `score_with_investigator()` 与独立 Chinese Brief Writer seam。先验证 20 个 case 的严格契约：

```bash
python3 -m pipeline.decision.run_layer2_component_eval --validate-only
```

实际 paired v1/v2 运行至少执行每个版本 3 个隔离、uncached trials，并要求显式提供 provider factory，避免把旧的 fixture-authored schema smoke 误当成 Agent eval：

Provider factory 必须为每个 trial 返回新实例，并显式声明 `eval_cache_mode = "disabled"`；如果 provider 自带缓存，则声明 `eval_cache_mode = "isolated_namespace"` 和每个 trial 唯一的 `eval_cache_namespace`。Runner 会验证并把该隔离证明写入 artifact。

```bash
python3 -m pipeline.decision.run_layer2_component_eval \
  --provider-factory your_module:create_provider \
  --trials 3 \
  --output-dir evals/layer2/results/my-run
```

输出包含 versioned per-case JSONL、aggregate JSON、Markdown v1/v2 diff、run metadata，以及需要 Brief 的 blind-comparison artifacts。退出码 `2` 表示 dataset/config 错误，`3` 表示执行或 artifact 错误，`4` 表示执行完成但 grader 未通过。
